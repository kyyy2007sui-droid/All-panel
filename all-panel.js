...
// ================= CONFIG =================
const BOT_TOKEN = "8927844672:AAGaQ8pS46gjvYit3cuMw-DM2Zy3YHCpUB8";
const OWNER_IDS = ["6614829903"];
const GRUP_OTP_LINK = "https://t.me/vanillaotp";
const GET_NUMBER_BOT = "https://t.me/vanilla_file";
const CHECK_INTERVAL = 3000;
const AUTO_DELETE_MS = 500000;
...
async function sendOTP(num, otp, cli, date, panel) {
  try {
    ...
    const text = `<code>${flag} #SN #${cs} | ${masked}</code>`;
    ...
    for (const [uid, s] of Object.entries(userSessions)) {
      if (s.active && s.active.includes(num) && s.cid) {
        const ut = `<code>${flag} #SN #${cs} | ${masked}</code>`;
